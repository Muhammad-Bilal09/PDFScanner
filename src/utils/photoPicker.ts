...
import { PickedPhotoResult } from '@/types/types';
...
async function checkMediaPermissions(): Promise<boolean> {
  ...
  if (!isPermitted && !perm.canAskAgain) {
    showAlert(
      ...
      'Scanly needs access to your photo library to import images. Please grant permission in Settings.',
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: () => {
            Linking.openSettings().catch(() => {
              showAlert('Error', 'Unable to open settings automatically.');
            });
          },
        },
      ]
